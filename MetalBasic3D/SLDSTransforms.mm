...
float SLDS::radians(const float& degrees)
{
...
simd::float4x4 SLDS::scale(const float& x,
                           const float& y,
                           const float& z)
{
...
simd::float4x4 SLDS::scale(const simd::float3& s)
{
...
simd::float4x4 SLDS::translate(const simd::float3& t)
{
...
simd::float4x4 SLDS::translate(const float& x,
                               const float& y,
                               const float& z)
{
    return SLDS::translate((simd::float3){x,y,z});
} // translate

#pragma mark -
#pragma mark Public - Transformations - Rotate

static float SLDSRadiansOverPi(const float& degrees)
{
    return (degrees * k1Div180_f);
} // SLDSRadiansOverPi

simd::float4x4 SLDS::rotate(const float& angle,
                            const simd::float3& r)
{
    float a = SLDSRadiansOverPi(angle);
    float c = 0.0f;
    float s = 0.0f;
                            ...
                            const float& z)
{
    ...
    return SLDS::rotate(angle, r);
} // rotate

#pragma mark -
#pragma mark Public - Transformations - Perspective

simd::float4x4 SLDS::perspective(const float& width,
                                 const float& height,
                                 const float& near,
                                 const float& far)
{
...
simd::float4x4 SLDS::perspective_fov(const float& fovy,
                                     const float& aspect,
                                     const float& near,
                                     const float& far)
{
    float angle  = SLDS::radians(0.5f * fovy);
                                     ...
                                     const float& far)
{
    ...
    return SLDS::perspective_fov(fovy, aspect, near, far);
} // perspective_fov

#pragma mark -
#pragma mark Public - Transformations - LookAt

simd::float4x4 SLDS::lookAt(const simd::float3& eye,
...
simd::float4x4 SLDS::lookAt(const float * const pEye,
                            ...
                            const float * const pUp)
{
    ...
    return SLDS::lookAt(eye, center, up);
} // lookAt

#pragma mark -
#pragma mark Public - Transformations - Orthographic

simd::float4x4 SLDS::ortho2d(const float& left,
                             const float& right,
                             const float& bottom,
                             const float& top,
                             const float& near,
...
simd::float4x4 SLDS::ortho2d(const simd::float3& origin,
                             const simd::float3& size)
{
    return SLDS::ortho2d(origin.x, origin.y, origin.z, size.x, size.y, size.z);
} // ortho2d

#pragma mark -
#pragma mark Public - Transformations - Off-Center Orthographic

simd::float4x4 SLDS::ortho2d_oc(const float& left,
                                const float& right,
                                const float& bottom,
                                const float& top,
                                const float& near,
...
simd::float4x4 SLDS::ortho2d_oc(const simd::float3& origin,
                                const simd::float3& size)
{
    return SLDS::ortho2d_oc(origin.x, origin.y, origin.z, size.x, size.y, size.z);
} // ortho2d_oc

#pragma mark -
#pragma mark Public - Transformations - frustum

simd::float4x4 SLDS::frustum(const float& fovH,
                             const float& fovV,
                             const float& near,
                             const float& far)
{
    float width  = 1.0f / std::tan(SLDS::radians(0.5f * fovH));
    float height = 1.0f / std::tan(SLDS::radians(0.5f * fovV));
...
simd::float4x4 SLDS::frustum(const float& left,
                             const float& right,
                             const float& bottom,
                             const float& top,
                             const float& near,
...
simd::float4x4 SLDS::frustum_oc(const float& left,
                                const float& right,
                                const float& bottom,
                                const float& top,
                                const float& near,
