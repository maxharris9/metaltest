...
static const float kBlubVertexData[] =
{
    kWidth, -kHeight, kDepth,   0.0, -1.0,  0.0,
    -kWidth, -kHeight, kDepth,   0.0, -1.0, 0.0,
    -kWidth, -kHeight, -kDepth,   0.0, -1.0,  0.0,
...
- (BOOL)preparePipelineState:(SLDSView *)view
{
...
//        MTLRegion region = MTLRegionMake2D(_depthTex.width/4, _depthTex.height/4, 1, 1);
//        
//        float depth;
...
- (void)updateConstantBuffer
{
    ...
    for (int i = 0; i < kNumberOfBoxes; i++)
    {
        ...
        simd::float4x4 modelViewMatrix = SLDS::translate(multiplier*0.1f, multiplier*0.1f, multiplier*0.5f); /* * SLDS::rotate(_rotation, 1.0f, 1.0f, 1.0f); */
